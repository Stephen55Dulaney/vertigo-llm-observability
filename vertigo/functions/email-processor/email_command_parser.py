...
''',
    ...
    def handle_list_this_week(self, subject: str, body: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # Handle None responses from Firestore errors
            if transcript_stats is None:
                return self.create_error_response("Unable to retrieve transcript statistics. Please try again later.")
            
            if meeting_stats is None:
                return self.create_error_response("Unable to retrieve meeting statistics. Please try again later.")
            
            response = "📊 Last 7 Days Summary\n"
            response += "=" * 30 + "\n\n"
            
            response += "📝 Transcripts:\n"
            if transcript_stats['projects']:
                for project, count in transcript_stats['projects'].items():
                    response += f"• {project}: {count} transcripts\n"
            else:
                response += "• No transcripts found\n"
            
            response += f"\n📅 Total Transcripts: {transcript_stats['total']}\n"
            response += f"📊 Success Rate: {transcript_stats['success_rate']:.1f}%\n"
            
            response += "\n🤝 Meetings:\n"
            if meeting_stats['projects']:
    ...
    def handle_list_projects(self, subject: str, body: str) -> Dict[str, Any]:
        ...
        try:
            ...
            response += f"\n📊 Summary:\n"
            all_projects = set(list(transcript_stats['projects'].keys()) + list(meeting_stats['projects'].keys()))
            response += f"• Total Projects: {len(all_projects)}\n"
